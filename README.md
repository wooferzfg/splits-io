...
## Local Development
Splits.io runs on Docker Swarm, which makes it easy and consistent to set up and run on any machine despite any unusual
...
```sh
docker stack deploy splitsio -c docker-compose.yml
docker service logs splitsio_web --raw --follow
```
...
In development PostgreSQL and S3 are also Docker containers. Lambda is not yet
...
```sh
docker run glacials/splitsio rails generate favicon
```
